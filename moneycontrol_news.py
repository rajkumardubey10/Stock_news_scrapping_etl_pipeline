...
# Configure logging to log to a file with timestamped entries
# logging.basicConfig(
#     filename='logs/scrape_news.log',
#     level=logging.INFO,
#     format='%(asctime)s - %(levelname)s - %(message)s'
# )

# Define the directory for scraping logs
scrapping_log_dir = os.path.join('logs', 'Money_scrapping_logs')
...
async def scrape_article_details(item, header, data):
    try:
        ...
        # Extract publish date, time, author, and full text
        schedule_date = article_soup.select_one('div.article_schedule span')
        publish_date = schedule_date.text if schedule_date else None

        schedule_time = article_soup.select_one('div.article_schedule')
        publish_time = schedule_time.text.split('/')[1] if schedule_time else None

        author = article_soup.select_one('div.article_author')
        author = author.text if author else "Money Control"  # Default author

        paragraph = article_soup.select('div.arti-flow p')
...
async def scrape_news():
    ...
    try:
        for page in range(1, 3):  # Modify the range as needed for more pages
            ...
            list_item = soup.select('li.clearfix')

            # Create tasks for each article to scrape asynchronously
            tasks = []
            for item in list_item:
                tasks.append(scrape_article_details(item, header, data))
...
def insert_data_to_db(data):
    ...
    try:
        for record in data:
            cursor.execute("""
                INSERT INTO public.news_data(title, title_link, image_link, description, publish_date, publish_time, author, full_text, scrapedata_inserted_at)
